...
public class BookingService {
    ...
    public ArrayList<Map<String, String>> listAvailableAppointments(String selected_date, AppointmentType type, Integer staff_id) throws ParseException {

        //Create a new Map object, called free slots
        ArrayList<Map<String, String>> freeSlots = new ArrayList<>();
...
//        cal.setTime(date_formatter.parse(selected_date));
        ...
        List<Appointment> bookedAppointments = appointmentRepository.findByDateAndStaffId(selected_date, staff_id);// To fetch booked appointments only once and passes them wherever we need to
        ...
        for (int i = 0; i < possible_startTimes.length; i++) { //loop to go through the possible start times Array and return each value as "hour"
            ...
            if (timeSlotAvailable(cal, type, bookedAppointments)) {
                Calendar end_timeApt = (Calendar) cal.clone(); //Set the end time adding the suration of the appointment
                end_timeApt.add(Calendar.MINUTE, appoinment_duration_minutes.get(type));
                //availableTimesForDay.add(time);
                
                String end_time = time_formatter.format(end_timeApt.getTime());//convert end time into a String
                
                Map<String, String> timeSlot = new HashMap<>(); //Create new map to store start and end times values
                
                timeSlot.put("start_time",time);//Adding start time to Map (Stringformat)
                timeSlot.put("end_time",end_time);//Adding end time to Map (String Format)
                
                freeSlots.add(timeSlot); //Adding the Map to ur ArrayList
            }
